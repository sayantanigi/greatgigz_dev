  ...
  function applicantlist_fetchdata($limit, $start, $job_title, $start_date,$end_date)
{
 ...
 if($data->num_rows() > 0)
 {
  foreach($data->result() as $row)
  {
   ...
   {
    $jobstatus='<td style="vertical-align: text-top;"><span class="btn btn-danger btn-sm" onclick="return change_status('.$row->appliedjob_id.')">Pending</span></td>';
   }
   else if($row->job_status=='shortlist'){
    $jobstatus='<td style="vertical-align: text-top;"><span class="btn btn-warning btn-sm" onclick="return change_status('.$row->appliedjob_id.')">Short List</span></td>';
   }
    else if($row->job_status=='finalselected'){
    $jobstatus='<td style="vertical-align: text-top;"><span class="work-time">Selected</span></td>';
   }
   else if($row->job_status=='rejected'){
    $jobstatus='<td style="vertical-align: text-top;"><span class="btn btn-danger btn-sm">Rejected</span></td>';
   }
   $output .= ' <tr>
