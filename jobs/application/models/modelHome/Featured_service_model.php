...
class Featured_service_model extends CI_Model
{

	var $column_order = array(null,'featured_service.image','featured_service.title','featured_service.description',null); //set column field database for datatable orderable
   // var $column_search = array('ms.country_name','md.state_name','mc.city_name','mc.status'); //set column field database for datatable searchable 
	...
	{
		$this->db->select('featured_service.*');
        ...
        if($_POST['search']['value']) // if datatable send POST for search
            {
                ...
                {  
                    ...
                    $cond.=" OR  featured_service.status LIKE '%".trim($show_string)."%') ";
                    $this->db->where($cond);
                }
            }
        $i++;
     ...
     function get_unique_url($url, $id = false)
    {
        ...
        $rest = $this->db->get('featured_service');
        if ($rest->num_rows() == 0) {
            return $url;
        } else {
            $cr = $rest->first_row();
