					...
					<div class="col-md-12">
               		...
               		<div class="form-group col-md-4 p-l">
									...
									<select class="form-control selectpicker" name="skill_id[]" multiple data-live-search="true"required >
											 ...
											 <option value="<?= $key->id?>" style="color:black;"><?= ucfirst($key->skill)?></option>
											<?php } }?>
										</select>
						...
						<input type="hidden" name="employer_subscription_id" value="<?= @$get_employer->id ?>">
							<?php if(date('Y-m-d',strtotime(@$get_employer->end_date)) > date('Y-m-d') && @$get_employer->no_of_post > count(@$total_postjobs)){?>
