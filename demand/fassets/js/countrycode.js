  ...
  Plugin.prototype = {
    ...
    _setInstanceCountryData: function () {
      ...
      if (this.options.onlyCountries.length) {
        ...
        for (i = 0; i < allCountries.length; i++) {
          if (
            $.inArray(allCountries[i].iso2, this.options.onlyCountries) != -1
          ) {
      ...
      this.telInput.on("keyup" + this.ns, function (e) {
        ...
        if (e.which == keys.ENTER || that.telInput.prop("readonly")) {
          ...
          var isCtrl =
            ...
            noSelection =
              that.isGoodBrowser && input.selectionStart == input.selectionEnd, // cursorAtEnd defaults to false for bad browsers else they would never get a reformat on delete
            cursorAtEnd =
              that.isGoodBrowser &&
              input.selectionStart == that.telInput.val().length;
          ...
          if (!that.options.nationalMode) {
            ...
            if (val.substr(0, 1) != "+") {
              ...
              var newCursorPos = that.isGoodBrowser
    ...
    _handleInputKey: function (newNumericChar, addSuffix) {
      ...
      if (this.isGoodBrowser) {
        ...
        if (newNumericChar) {
          ...
          val =
            val.substr(0, input.selectionStart) +
            newNumericChar +
      ...
      if (
        ...
        val.charAt(0) == "+" &&
        numeric.length &&
        !this._hasPotentialDialCode(numeric, this.pcc, 0)
      ) {
      ...
      if (this.isGoodBrowser) {
        ...
        if (!digitsOnRight) {
          ...
          if (!newNumericChar) {
            newCursor = this._getCursorFromLeftChar(
              val,
              newCursor,
              originalLeftChar
            );
    ...
    _initFocusListeners: function () {
        ...
        if (
          ...
          that.telInput.one("keypress.plus" + that.ns, function (e) {
            if (e.which == keys.PLUS) {
              ...
              var newVal =
                that.options.autoFormat && window.intlTelInputUtils ? "+" : "";
      ...
      this.telInput.on("blur" + this.ns, function () {
        ...
        if (
          that.options.autoFormat &&
          window.intlTelInputUtils &&
          that.telInput.val() != that.telInput.data("focusVal")
        ) {
    ...
    _updateVal: function (val, addSuffix) {
      ...
      if (this.options.autoFormat && window.intlTelInputUtils) {
        formatted = intlTelInputUtils.formatNumber(
          val,
    ...
    _updateFlagFromNumber: function (number) {
      ...
      if (dialCode) {
        ...
        var countryCodes = this.countryCodes[this._getNumeric(dialCode)],
          alreadySelected =
            this.selectedCountryData &&
            $.inArray(this.selectedCountryData.iso2, countryCodes) != -1;
    ...
    _updateDialCode: function (newDialCode, focusing) {
      ...
      if (this.options.nationalMode && inputVal.substr(0, 1) != "+") {
        ...
        if (prevDialCode.length > 1) {
          ...
          var existingNumber =
            inputVal.substr(0, 1) != "+" ? $.trim(inputVal) : "";
    ...
    getNumber: function (type) {
      if (window.intlTelInputUtils) {
        return intlTelInputUtils.formatNumberByType(
          this.telInput.val(),
          this.selectedCountryData.iso2,
          type
        );
