...
if(isset($_GET['del'])){
	$del2=$_GET['del'];
   $r= $mysqli->query("select * from bsi_adminmenu where parent_id=".$mysqli->real_escape_string($_REQUEST['id']));
   //if(@mysql_num_rows($r)){
   if($r->num_rows){
      $msg="<font face=verdana size=1 color=#ffffff><b>You Can't delete.agentmenus exists in this category.</b></font>";	  
   }else{
      $r= $mysqli->query("delete from bsi_adminmenu where id=\"" .$mysqli->real_escape_string($_REQUEST['id']). "\"");
...
$r=$mysqli->query("select * from bsi_adminmenu where parent_id=0 order by ord");
	...
	echo"<tr bgcolor=#ffffff class=even>
			<td >";
				if(in_array($d['id'],$tids)):
					...
					$q=$mysqli->query("select count(*) as total from bsi_adminmenu where  parent_id=" . $d['id']);
    	...
    	if(in_array($d['id'],$tids)):
			$rr=$mysqli->query("select * from bsi_adminmenu where parent_id=" . $d['id'] . " order by ord");
			$k=0;
