 ...
 */
 ...
 
 {
	...
	private function advancePayment(){
		...
		$result = $mysqli->query("SELECT * FROM bsi_advance_payment WHERE month_num = ".$month);
 
		$this->depositPlans = $result->fetch_assoc();
	...
	public function generateBookingDetails() {
		...
		//print_r($this->selectedRooms);die;
		...
		for($i = 0; $i < $selectedRoomsCount; $i++){
			...
			if($this->selectedRooms[$i] > 0){		
				...
				$child_flag2=false;
				$sql=$mysqli->query("SELECT distinct(`no_of_child`) FROM `bsi_room` WHERE `capacity_id`=".$this->detailVars[$dvarsCtr]['capacityid']." and `roomtype_id`=".$this->detailVars[$dvarsCtr]['roomtypeid']."");
				$chld_row2=$sql->fetch_assoc();
