...
if($id){
	$text     = '';
	$start_dt = $mysqli->real_escape_string($_REQUEST['start_dt']);
	if($start_dt != '0000-00-00'){
		$sql=$mysqli->query("SELECT DATE_FORMAT(start_date, '".$bsiCore->userDateFormat."') AS start_date1,
	...
	$sql1=$mysqli->query("select * from bsi_roomtype where roomtype_ID='".$row['roomtype_id']."'");
	$rtypeName = $sql1->fetch_assoc();
