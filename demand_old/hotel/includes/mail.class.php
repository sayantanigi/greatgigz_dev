...
class bsiMail 
{
	...
	function __construct(){
		...
		if($this->isSMTP == true){	
			...
			$this->emailFrom = $bsiCore->config['conf_hotel_name']."<info@prosearchghana.com>";		
		}else{
		...
		$this->smtpHost 		= "ssl://mail.prosearchghana.com";
		$this->smtpPort 		= intval(465);
		$this->smtpUserName 	= "info@prosearchghana.com";
		$this->smtpPassword 	= "KAL@prosearch2020";
	...
	public function loadEmailContent() {	
		global $mysqli;
		$sql = $mysqli->query("SELECT * FROM bsi_email_contents WHERE email_name = 'Confirmation Email'");
		$currentrow = $sql->fetch_assoc();	
