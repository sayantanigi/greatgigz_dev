...
class Dashboard extends CI_Controller {
	...
	function index() {
		$data['get_service'] = $this->Crud_model->GetData('employer_services', '', "employer_id='" . $_SESSION['commonUser']['userId'] . "'");
		$data['get_job'] = $this->Crud_model->GetData('postjob', '', "user_id='".$_SESSION['commonUser']['userId']."'");
		$data['bid_job'] = $this->db->query("SELECT `postjob`.*, `job_bid`.* FROM `job_bid` JOIN `postjob` ON `postjob`.`id` = `job_bid`.`postjob_id` where `postjob`.user_id = '".$_SESSION['commonUser']['userId']."' AND postjob.is_delete = '0'")->result_array();
		$data['get_subscribe'] = $this->Crud_model->GetData('employer_subscription', '', "employer_id='" . $_SESSION['commonUser']['userId'] . "'");
		$data['get_user'] = $this->Crud_model->get_single('users', "userId ='" . $_SESSION['commonUser']['userId'] . "' and userType='1'");
		$data['get_product'] = $this->Crud_model->GetData('user_product', '', "user_id='".$_SESSION['commonUser']['userId']."' AND status = 1 AND is_delete= 1");
	...
	public function view_profile() {
		$user_info = $this->Crud_model->get_single('users', "userId='" . $_SESSION['commonUser']['userId'] . "'");
		$data = array(
			'userinfo' => $user_info,
		);
		$this->load->view('header');
	...
	public function profile() {
		...
		// } else {
		// 	$userid=$_SESSION['commonUser']['userId'];
		// 	$data_request='user';
		// 	$this->load->view('header');
		// }
		$userid=$_SESSION['commonUser']['userId'];
	...
	public function update_profile() {
		...
		$this->session->set_flashdata('message', 'Profile Updated Successfull !');
		redirect(base_url('projects/profile'));
		}
	}
	...
	public function subscription() {
		...
		$data['current_plan'] = $this->Crud_model->GetData('employer_subscription', '', "employer_id='".$_SESSION['commonUser']['userId']."' AND status IN (1,2)");
		$data['expired_plan'] = $this->Crud_model->GetData('employer_subscription', '', "employer_id='".$_SESSION['commonUser']['userId']."' AND status = '3'");
		$data['subscription_check'] = $this->db->query("SELECT * FROM employer_subscription WHERE employer_id='".$_SESSION['commonUser']['userId']."' AND (status = '1' OR status = '2')")->result_array();
		$this->load->view('header');
	...
	public function products() {
		$data['product_list'] = $this->Crud_model->GetData('user_product', '', "user_id='".$_SESSION['commonUser']['userId']."' AND status = 1 and is_delete = 1");
		$this->load->view('header');
	...
	public function myservice() {
		$data['get_services'] = $this->Crud_model->GetData('employer_services', '', "employer_id='" . $_SESSION['commonUser']['userId'] . "'");
		$this->load->view('header');
	...
	public function myjob() {
		$data['get_postjob'] = $this->Crud_model->GetData('postjob', '', "user_id='".$_SESSION['commonUser']['userId']."' ");
		//print_r($data); die();
		$this->load->view('header');
	...
	function jobbid() {
		$this->load->model('Post_job_model');
		if($_SESSION['commonUser']['userType'] == '1'){
			//$data['get_postjob'] = $this->db->query("SELECT postjob.*, job_bid.*, users.* from job_bid JOIN postjob ON job_bid.postjob_id = postjob.id JOIN users ON job_bid.user_id = users.userId WHERE postjob.user_id ='".$_SESSION['commonUser']['userId']."'")
			$cond = "job_bid.user_id='" . $_SESSION['commonUser']['userId'] . "'";
		} else {
			...
			$cond = "postjob.user_id='" . $_SESSION['commonUser']['userId'] . "'";
		}
	...
	function showmessage_list() {
		$userdId = $_SESSION['commonUser']['userId'];
		...
		if (!empty($get_data)) {
			foreach ($get_data as $key) {
				...
				if ($key->userfrom_id == $_SESSION['commonUser']['userId'] && $key->userto_id == $_POST['usert_id'] && $key->postjob_id == $_POST['post_id']) {
					$sent = '<li class="sent">' . $from_pic . '<p>' . $key->message . '</p><div style="font-size: 10px;">'.$key->created_date.'</li>';
				} else {
					$sent = '';
				}
				if ($key->userto_id == $_SESSION['commonUser']['userId'] && $key->userfrom_id == $_POST['usert_id'] && $key->postjob_id == $_POST['post_id']) {
	...
	function showmessage_listS() {
		...
		if (!empty($get_data)) {
			foreach ($get_data as $key) {
				...
				if ($key->userfrom_id == $_SESSION['commonUser']['userId'] && $key->userto_id == $user_id && $key->postjob_id == $post_id) {
					$sent = '<li class="sent">' . $from_pic . '<p>' . $key->message . '</p><div style="font-size: 10px;">'.$key->created_date.'</li>';
				} else {
					$sent = '';
				}
				if ($key->userto_id == $_SESSION['commonUser']['userId'] && $key->userfrom_id == $user_id && $key->postjob_id == $post_id) {
					$reply = '<li class="replies">' . $to_pic . '<p>' . $key->message . '</p><div style="font-size: 10px;">'.$key->created_date.'</li>';
				} else {
					$reply = '';
				}
	...
	public function get_events() {
		$events = $this->db->query("select * from appointment_scheduling where user_id='" . $_SESSION['commonUser']['userId'] . "'")->result();
		$data_events = array();
	...
	{
		$data['education_list'] = $this->Crud_model->GetData('user_education', '', "user_id='".$_SESSION['commonUser']['userId']."' order by id DESC");
		$this->load->view('header');
	...
	function workexperience_list() {
		$data['workexperience_list'] = $this->Crud_model->GetData('user_workexperience', '', "user_id='".$_SESSION['commonUser']['userId']."' order by id DESC");
		$this->load->view('header');
