...
class Home extends MY_Controller {
	...
	public function index() {
		$data['get_post'] = $this->Crud_model->GetData('postjob', 'id,job_title,description,user_id', "is_delete='0'", '', '(id)desc', '6');
	...
	function pricing() {
		...
		$data['subcriber_pack'] = $this->Crud_model->GetData('employer_subscription', '', "employer_id='" . @$_SESSION['commonUser']['userId'] . "'");
	...
	function post_bidding($postid) {
		...
		if(!empty($_SESSION['afrebay_admin'])){
			$type='admin';
		} else if(!empty($_SESSION['commonUser'])) {
			$type='user';
		} else {
			$type='nouser';
		}
			...
			foreach ($getfilterData as $key) {
				...
				<div class='col-lg-3 col-md-6 col-sm-6 col-xs-12'>
					<div class='pricetable style2'>
						...
						if(!empty($_SESSION['commonUser']['userType'])) {
							$subcriber_pack = $this->Crud_model->GetData('employer_subscription', '', "employer_id='" . @$_SESSION['commonUser']['userId'] . "'");
							if(!empty($subcriber_pack)) {
								...
								if($key['subscription_type'] == 'paid') {
									...
									$html .= "<input type='hidden' name='user_id_".$key['id']." id='user_id_".$key['id']." value=".$_SESSION['commonUser']['userId'].">";
									...
									$html .= "<input type='hidden' name='user_email_".$key['id']." id='user_email_".$key['id']." value=".$_SESSION['commonUser']['userEmail'].">";
