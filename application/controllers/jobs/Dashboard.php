...
public function __construct()
  ...
  function index()
  {
    $data['getuser']=$this->Crud_model->get_single('users',"userId='".$_SESSION['commonUser']['userId']."' and  status='1'");
    ...
    $count = count($this->input->post('education',TRUE));
         for ($i=0; $i < $count; $i++)
        {
          $update_class=array(
            ...
            'university_institute'=>$this->input->post('university_institute',TRUE)[$i],
            'marks'=>$this->input->post('marks',TRUE)[$i],
            'year'=>$this->input->post('year',TRUE)[$i],
         ...
         for ($i=0; $i < $count1; $i++)
        {
          $update_workexperience=array(
            'user_id'=>$this->input->post('userid',TRUE),
             'employer_name'=>$this->input->post('employer_name',TRUE)[$i],
            'status'=>$this->input->post('status',TRUE)[$i],
            'start_date'=>$this->input->post('start_date',TRUE)[$i],
            'end_date'=>$this->input->post('end_date',TRUE)[$i],
            'designation'=>$this->input->post('designation',TRUE)[$i],
            'job_profile'=>$this->input->post('job_profile',TRUE)[$i],
            'created_date'=>date('Y-m-d H:i:s'),
          );
