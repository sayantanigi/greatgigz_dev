                                        ...
                                        <div id="frame">
                                            <div id="sidepanel">
                                                ...
                                                <div id="contacts">
                                                    ...
                                                    foreach ($get_jobbid as $user) {
                                                        //if ($user->postjob_id == $user->post_id && $user->user_id == $_SESSION['commonUser']['userId'] && $user->bidding_status == 'Accept') {
                                                        if ($user->postjob_id == $user->post_id && $user->user_id == $_SESSION['commonUser']['userId'] && ($user->bidding_status == 'Selected' || $user->bidding_status == 'Short Listed')) {
                                                        ...
                                                        <li class="contact" onclick="return getuser('<?= $user->userid ?>','<?= $user->post_id ?>');">
                                                            <div class="wrap">
                                                                ...
                                                                <div class="meta">
                                                                    ...
                                                                    <?php
                                                                    //$countMessage = $this->db->query("Select COUNT(id) as msgcount FROM chat WHERE userto_id ='".$_SESSION['commonUser']['userId']."' AND status = '0' AND postjob_id = '".$user->post_id."'")->result();
                                                        ...
                                                        <?php } else if ($user->postjob_id == $user->post_id && $user->userid == $_SESSION['commonUser']['userId'] && ($user->bidding_status == 'Selected' || $user->bidding_status == 'Short Listed')) {
                                            ...
                                            <div class="content">
                                                ...
                                                <div class="message-input">
                                                    <div class="wrap">
                                                        <input type="hidden" name="userfromid" id="userfromid" value="<?= $_SESSION['commonUser']['userId']?>" />
...
<div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
    <div class="modal-dialog modal-sm" role="document" style="max-width: 100%;">
        ...
        <?php
        $myBidList =  $this->db->query("SELECT postjob.job_title FROM job_bid JOIN postjob on job_bid.postjob_id = postjob.id where job_bid.user_id = '".$_SESSION['commonUser']['userId']."' and job_bid.bidding_status = 'Accept' and job_bid.status = 'Active'")->result_array();
        if(!empty($myBidList)) {
        $i = 1;
        foreach ($myBidList as $value) { ?>
            <p><?= $i.". ".$value['job_title']?></p>
        <?php $i++; } } ?>
        </div>
    </div>
</div>
